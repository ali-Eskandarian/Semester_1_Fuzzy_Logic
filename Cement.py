...
def main(fuzzy_type, output_col, mode = "All features", scale = "Minmax", fuzzy_shape = "All gaussian", number_of_fuzzy_sets = "optimize"):
    ...
    regressor = FuzzyRegression(selected_input_columns, output_col,std_factor=0.25, fuzzy_type=fuzzy_type, fuzzy_shape=fuzzy_shape, number_of_fuzzy_sets=number_of_fuzzy_sets)
    best_rmse_result, best_prediction, best_actual, rules, fuzzy_vars, avg_rmse, sk_pred, r2_score = evaluate_model(
        regressor, data, selected_input_columns, output_col, fuzzy_type, scale, num_iterations=30
    )
    
    plot_fuzzy_sets(data, fuzzy_vars, selected_input_columns, fuzzy_type, folder = "plots_2", scaled=scale)
    plt.close()
    ...
    plot_predictions_vs_actuals(best_actual, best_prediction, sk_pred, output_col, fuzzy_type , folder = "plots_2")
    plt.close()
...
if __name__ == "__main__":
    data = pd.read_excel('concrete+slump+data.xlsx')
    input_columns = ["Cement", "Slag", "Fly ash", "Water", "SP", "Coarse Aggr.", "Fine Aggr."]
    output_columns = ["Compressive Strength (28-day)(Mpa)", "SLUMP(cm)", "FLOW(cm)" ]
    
    scales = ["Nothing" ]#,"Minmax", "Nothing","Robust"]
    fuzzy_shapes = ["All gaussian"]#, "Both Gaussian and Trapezoidal"]
    fuzzy_types_to_use = ['type1']#,'type1']
    results = {}
        ...
        for fuzzy_type in fuzzy_types_to_use:
            ...
            for k, scale in enumerate(scales):
                for l, fuzzy_shape in enumerate(fuzzy_shapes):
                    best_rmse, avg_rmse, r2_score = main(fuzzy_type, output_col, mode="All features",
                                                         scale=scale, fuzzy_shape=fuzzy_shape,
    ...
    # Print best configurations for each output column
    print("\nBest Configurations:")
    for output_col in output_columns:
        print(f"\nFor {output_col}:")
        for fuzzy_type in fuzzy_types_to_use:
            best_config = min(results[output_col][fuzzy_type].items(), 
                            key=lambda x: x[1][1])  # x[1][1] is avg_rmse
            scale, shape = best_config[0].split('_')
            print(f"  {fuzzy_type}: Scale={scale}, Shape={shape}, Avg RMSE={best_config[1][1]:.4f}")

    plot_performance_metrics_all(output_columns, results, scales, fuzzy_shapes,fuzzy_types_to_use)
